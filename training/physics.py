...
def compute_mbe_lhs(y_t, y_tp1, phi_m, phi_frac, *,
    ...
    exp_m = (c_f * (P_m - P_ref) - beta * (T_m - T_ref)).clamp(*exp_clip)
    exp_f = (c_f * (P_f - P_ref) - beta * (T_f - T_ref)).clamp(*exp_clip)
...
def compute_mbe_penalty(y_t, y_tp1, action_normalized, phi_m, phi_frac, *,
                        pres_min, pres_max, char_mass=1e7, huber_beta=1.0,
                        reduction="mean"):
    """Return the raw Smooth L1 penalty for the normalized MBE residual."""
...
def compute_mbe_loss(y_t, y_tp1, target_t, target_tp1,
                     action_normalized, phi_m, phi_frac, *,
                     pres_min, pres_max, char_mass=1e7, huber_beta=1.0):
    """Penalize per-sample MBE error beyond the true transition's penalty."""
    pred_penalty = compute_mbe_penalty(
        y_t, y_tp1, action_normalized, phi_m, phi_frac,
        pres_min=pres_min, pres_max=pres_max,
        char_mass=char_mass, huber_beta=huber_beta,
        reduction="none",
    )

    with torch.no_grad():
        target_penalty = compute_mbe_penalty(
            target_t, target_tp1, action_normalized, phi_m, phi_frac,
            pres_min=pres_min, pres_max=pres_max,
            char_mass=char_mass, huber_beta=huber_beta,
            reduction="none",
        )

    return F.relu(pred_penalty - target_penalty).mean()
