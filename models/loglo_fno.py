...
from .aux_head import WellAuxHead
...
class LayerNorm3d(nn.Module):
    ...
    def __init__(self, num_channels, eps=1e-6, elementwise_affine=False):
        super().__init__()
        self.norm = nn.LayerNorm(
            num_channels, elementwise_affine=elementwise_affine, eps=eps
        )
...
class ModulatedLOGLO_FNO(nn.Module):
    def __init__(self, in_dim=4,
        ...
        self.final_norm = LayerNorm3d(num_channels=self.hidden_dim, elementwise_affine=True)
    ...
    def _operator_output(self, x, action):
    ...
    def forward(self, x, action):
        return self._operator_output(x, action) + x[:, :self.out_dim]


class ModulatedLOGLO_FNO_Aux(ModulatedLOGLO_FNO):
    """Modulated LOGLO-FNO with a shared per-well aux readout."""

    def __init__(self, aux_hidden=(128, 64), **kwargs):
        super().__init__(**kwargs)
        self.aux_head = WellAuxHead(
            state_channels=self.out_dim,
            static_channels=4,
            hidden=tuple(aux_hidden),
        )

    def forward(self, x, action):
        spatial_out = super().forward(x, action)
        y_t = x[:, :self.out_dim]
        static = x[:, self.out_dim:self.out_dim + 4]
        aux_out = self.aux_head(y_t, spatial_out, static, action[:, 0])
        return spatial_out, aux_out
